...
import com.vector.update_app.ext.UpdateAppInstance;
...
//        getDialog().getWindow().addFlags(WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL);
    ...
    private void initData() {
        if (getArguments() != null &&!getArguments().isEmpty()&& getArguments().getSerializable(UpdateAppManager.INTENT_KEY) != null) {
            mUpdateApp = (UpdateAppBean) getArguments().getSerializable(UpdateAppManager.INTENT_KEY);
        }
        ///通过序列化读取限制的 HttpManager 的编写，这里尝试直接从内存中拿数据
        if (mUpdateApp == null) {
            mUpdateApp = UpdateAppInstance.INSTANCE.getUpdateAppBean();
        }
        //设置主题色
        initTheme();
    ...
    private void initTheme() {
        ...
        int topResId = -1;
        if (arguments != null) {
            color = arguments.getInt(UpdateAppManager.THEME_KEY, -1);
            topResId = arguments.getInt(UpdateAppManager.TOP_IMAGE_KEY, -1);
        }
